...
public class DataAnalyzer 
{
   ...
   /**
    * Turns string into bare text (letters/numbers/spaces) so easier to compare to user entered game
    */
   public String clean(String aString)
   {
      return aString.replaceAll("//.|:|-","").toLowerCase();
   }

   public void saveTable(String tableName, String mapName)
   {
      try
      {
         ...
         for (Entry<String, Integer> entry : indexMap.entrySet())
         {
            ...
            out.write(",");
            out.write(((Integer) entry.getValue()).toString());
            out.write("\n");
         }
         out.close();
         ...
         {
            for (int j = 0; j < scoreTable[i].length - 1; j++)
            {
               if(scoreTable[i][j] != null)
                  out.write(((Double) scoreTable[i][j].total).toString());
               else out.write("-1");
               out.write(",");
            }
            int j = scoreTable[i].length - 1;
            if(scoreTable[i][j] != null)
               out.write(((Double) scoreTable[i][j].total).toString());
            else out.write("-1");
            out.write("|");
         }
         int i = scoreTable.length - 1;
         for (int j = 0; j < scoreTable[i].length - 1; j++)
         {
            if(scoreTable[i][j] != null)
               out.write(((Double) scoreTable[i][j].total).toString());
...
/*
   private void loadTable(String tablePath, String mapPath)
   {
      try
      {
         Scanner in = new Scanner(new File(mapPath));
         while (in.hasNextLine())
         {
            String line = in.nextLine();
            int comma = line.indexOf(",");
            indexMap.put(line.substring(0, comma), Integer.parseInt(line.substring(comma+1)));
         }
         in.close();

         in = new Scanner(new File(tablePath));
         String line = in.nextLine();
         ...
         for (int i = 0; i < row.length; i++)
         {
            ...
            {
               scoreTable[i][j] = Double.parseDouble(col[i][j]);
            }
         }
         in.close();
      }
